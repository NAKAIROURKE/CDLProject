...
(
  match_id INT,
  match_play_time bigint, --the jason data provided unix timestamps for our date played so we will keep it in that format and use CAST() whenever we want to use for analysis
  match_created_at bigint,
  home_team_games_won INT NOT NULL,
  away_team_games_won INT NOT NULL,
  home_team_id INT,
  away_team_id INT,
  winning_team_id INT,
  losing_team_id INT,
...
(
  match_map_id INT,
  home_score INT DEFAULT 0, --Set default values if score missing/ noted in error
  away_score INT DEFAULT 0,
  match_id INT, --WILL ADD NOT NULL SO THERE CANNOT BE FLOATING MAPS WITH NOT MATCH ID PAIRING
  map_id INT,
  winning_team_id INT,
  losing_team_id INT,
...
Alter table matchmaps 
Add column map_name VARCHAR (45); --Realized i cannot add map_id to this table if there is no way to relate the map_id without any context (name/mode/map_id)
--Will probably drop column after initial set up since then you can query both tables and they will have matching info
...
(
  ...
  friendly_damage  INT ,
  distance_traveled  numeric (10,3) ,
  average_speed numeric (7,3), --Seven digits total, three after the decimal
  control_tiers_captured INT  ,
  ...
  FOREIGN KEY (match_map_id) REFERENCES matchmaps(match_map_id),
...
SET team_id = (Select team_id
			  From team
			  Where	abv = 'FLA')
Where	lower(alias) IN ('havok','brack','felo','vikul','capsidal','majormaniak');
...
Where (home_score = 250 OR away_score = 250)
...
Where (home_score = 6 OR away_score = 6)
	AND (map_name LIKE '%otel');
...
Where (home_score = 3 OR away_score = 3)
	AND (map_name LIKE '%otel')
	AND home_score + away_score <6;
